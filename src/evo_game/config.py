...
def write_default_config(path: Path | str = "config.toml", overwrite: bool = False) -> Path:
    ...
    Returns:
        Path: Location of the written configuration file.
    """
    ...
    data = _stringify_paths(AppConfig().model_dump())
...
def _stringify_paths(raw: Dict[str, Any]) -> Dict[str, Any]:
    """Convert Path entries in a nested dict to strings for TOML writing."""
    ...
    for key, value in raw.items():
        if isinstance(value, Path):
            ...
            converted[key] = _stringify_paths(value)
        else:
            converted[key] = value
    return converted
